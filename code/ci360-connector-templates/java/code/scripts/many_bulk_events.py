...
def bulk_events(number_of_events: int):
  event_name = 'SpringTrigger'

  # Create event data
  event_data = '\n'.join(f'{event_name},subject_id,1660,text,"Hello world!",number,{number}' for number in range(number_of_events))
  #print(event_data)
...
# Send events for 1 hour
start_dttm = datetime.now()
end_dttm = start_dttm + timedelta(hours=1)

while datetime.now() < end_dttm:
  print(datetime.now(), 'stops at', end_dttm)
  try:
    bulk_events(150) # 5 tps
  except:
    pass
  time.sleep(30)
