...
TRACE:int=5
@staticmethod
def traceMethod(self, message, *args, **kws) -> None:
...
class SASCI360VeloxPyLogging():
    ...
    loggers:List["SASCI360VeloxPyLogging"] = []
    @classmethod
    def getLoggerCount(cls) -> int:
        """ 
        Returns the count of loggers created by this class.
        """
        return len(cls.loggers)
    
    @classmethod
    def getDefaultLogger(cls) -> "SASCI360VeloxPyLogging":
        """
        Returns the default logger (the first logger created) if it exists, otherwise returns None.
        """
        if cls.getLoggerCount() > 0:
            return cls.loggers[0]   
        return None
    @classmethod
    def getLoggers(cls) -> List[logging.Logger]:
        """
        Returns the list of loggers created by this class.
        """
        return cls.loggers
    @classmethod
    def getLoggerByNameOrIndex(cls, loggerName: str | int) -> logging.Logger:
        """
        Returns a logger by name or index if it exists, otherwise returns None.
        """
        if isinstance(loggerName, str):
            return cls.getLoggerByName(loggerName)
        elif isinstance(loggerName, int):
            return cls.getLoggerByIndex(loggerName)
        return None
    ...
    def initializeLogging(self,vAppConfigINI: str | configparser.ConfigParser = "sasci360veloxpy.ini", forceReInitializeLogger: bool = False):
        ...
        if self.__loggingConfig == None or forceReInitializeLogger:
            ...
            SASCI360VeloxPyLogging.loggers.append(self)
            #Define additional custom logging level
            logging.addLevelName(TRACE, "TRACE")
            logging.Logger.trace = traceMethod
    ...
    def writeLogMessage(self,vStrLogMessage: str, vLevel="INFO"):
        self.developerLog(vStrLogMessage=vStrLogMessage)
        try:
            if vStrLogMessage == None or vStrLogMessage == "":
    ...
    def info(self, vStrLogMessage: str):
        self.writeLogMessage(vStrLogMessage, "INFO")
    def debug(self, vStrLogMessage: str):
        self.writeLogMessage(vStrLogMessage, "DEBUG")   
    def warning(self, vStrLogMessage: str):
        self.writeLogMessage(vStrLogMessage, "WARN")    
    def error(self, vStrLogMessage: str):
        self.writeLogMessage(vStrLogMessage, "ERROR")   
    def critical(self, vStrLogMessage: str):
        self.writeLogMessage(vStrLogMessage, "CRITICAL")
    def trace(self, vStrLogMessage: str):
        self.writeLogMessage(vStrLogMessage, "TRACE")