...
/* ****************************************************************************/
%macro create_azure_code;
%let errFLag = 0;
%let code_file_path=&cdmcodes_path.&slash.azcode.sas;
/*%let get_Eventdttm="&UtilityLocation.&slash.macros&slash.get_Eventdttm.sas";*/
...
filename azcode DISK "&code_file_path.";
...
data _null_;
    ...
    put +6 'connect to SQLSVR (noprompt="uid=&dbuser;pwd=&dbpass;dsn=&dbdns;");'; 
	put +6 'execute (drop table ' %trim("&staging_table") ';) by SQLSVR;';
	put +4 'disconnect from SQLSVR;';
    put +4 'quit;'; 
	put+1 '%end;';
	
    put +4 'proc sql noerrorstop;';
 
    put +4 'connect to SQLSVR (noprompt="uid=&dbuser;pwd=&dbpass;dsn=&dbdns;");'; 
    put +6 'execute (create table ' %trim("&staging_table") ' (';
	if "&col_nm_key" ne '' then do;
	put+6 "&col_nm_key";
	put+6 ',';
	end;
	put +6 "&col_nm" ')) by SQLSVR;';
	if "&pk_col" ne '' then do;
		put +6 'execute (alter table ' %trim("&staging_table") ' add constraint ' %trim("&tmp_pk_name") ' primary key (' %trim("&tmp_pk_col") ')) by SQLSVR;';
	end;
	put +4 'disconnect from SQLSVR;';
    put +4 'quit;'; 
	...
	if "&pk_col" ne '' then do;
	    put +2 'proc sql noerrorstop;';
	    put +2 'connect to SQLSVR (noprompt="uid=&dbuser;pwd=&dbpass;dsn=&dbdns;");';   
	    put +4 'execute (merge into ' %trim("&table_name") ' using ' %trim("&staging_table");
	    put +6 ' on (' %trim("&merge_condition") ')';
	    put +6 'when matched then update';
	    put +6 'set ' %trim("&merge_update");
	    put +6 'when not matched then insert ( ' %trim("&merge_insert");
	    put +6 ' ) values ( ' %trim("&merge_value");
	    put +4 ' );) by SQLSVR;';
	    put +2 'run;';
	    put +2 'disconnect from SQLSVR;';
	    put +2 'quit;';
	    ...
	    put +2 'proc sql noerrorstop;';
	    put +2 'connect to SQLSVR (noprompt="uid=&dbuser;pwd=&dbpass;dsn=&dbdns;");'; 
	    put +4 'execute (drop table ' %trim("&staging_table") ') by SQLSVR;';
	    put +2 'disconnect from SQLSVR;';
	    put +2 'quit;';
		put +2 '%put ######## Staging table: ' %trim("&staging_table") ' Deleted ############;';
	  	...
	  	put +2 'proc sql noerrorstop;';
	  	put +2 'connect to SQLSVR (noprompt="uid=&dbuser;pwd=&dbpass;dsn=&dbdns;");';   
	  	put +4 'execute ( Truncate table '%trim("&table_name")'; ) by SQLSVR;';
	  	put +4 'execute ( Insert into '%trim("&table_name")' ('%trim("&merge_insert")') ';
	  	put +6 'select ' %trim("&merge_value") ' from '%trim("&staging_table") ';) by SQLSVR; ';
    	put +4 'execute (drop table '%trim("&staging_table") ';) by SQLSVR;';
    	put +2 'disconnect from SQLSVR;';
    	put +2 'quit;';
    ...
    put '%put------------------------------------------------------------------------------------------------------------;';
run;
/*%ErrorCheck(Unable to generate load code for &table_name);*/
/*%if &errFlag %then %do;*/
/*	%goto ERREXIT;*/
/*%end;*/
data _null_;
...
/**/
/*%ERREXIT:*/
/*    %put ERROR: create_azure_codeMacro encountered an error, exiting.;*/

%mend create_azure_code;