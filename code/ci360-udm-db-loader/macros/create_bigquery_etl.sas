...
/* ****************************************************************************/
%macro create_bigquery_etl(database=&database, table_name=,column_table=, key_list=) ;
	...
	data _null_;
		...
		put +2 '%let errFlag=0;';
		put +2 '%let nrows=0;';
		put +2 '%let dsid =%sysfunc(open(&udmmart..' "&table_name.));" ;
		put +2 '%let nrows=%sysfunc(attrn(&dsid,nlobs));';
		put +2 '%let dsid =%sysfunc(close(&dsid));';	
		put +2 '%if &nrows = 0 %then %do;';
        put +6 '%put NOTE: ' "&table_name." ' has 0 rows. Skipping load.;';
 		put +2 '%end;';
		put +2 '%else %do;';
		...
		data _null_;
			...
			if _n_=1 then do;
				* truncate table ;
				...
				put +3 'proc append data=&udmmart..' table_name ' base=&trglib..' table_name '(&DB_BL_OPTS) force;';
				put +3 'run;';
...
/*				put +3 'data work.' staging_table '/view=work.' staging_table ';';*/
			...
			if last then do;
				...
				put +9  'execute( create  table &tmpdbschema..' staging_table ' as ';
				put +12  'select * from &dbschema..' table_name ' where 1=0 ;';
				put +12  ') by &database.;';
				put +9  'disconnect from &database.;';
				put +6 'quit;';
				put +6 'proc append data=work.' staging_table ' base=&tmplib..' staging_table '(&DB_BL_OPTS) force;';
