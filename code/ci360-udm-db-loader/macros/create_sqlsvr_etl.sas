...
%macro create_sqlsvr_etl(database=&database., table_name=, column_table=, key_list=);
    ...
    /* SQL Server uses INSERT/SELECT instead of PROC APPEND for row-level  */
    /* inserts; bulkload threshold triggers PROC APPEND with BULKLOAD=Yes  */
    /* ------------------------------------------------------------------ */
    %if &key_list. = " " %then %do;
        DATA _NULL_;
            ...
            IF _n_ = 1 THEN DO;
                ...
                * Use bulkload for large tables, INSERT/SELECT for small ones;
                PUT +3 '%if &nrows ge &DB_BL_THRESHOLD. and &DB_BL_THRESHOLD. gt 0 %then %do;';
                PUT +6 'PROC APPEND DATA=&udmmart..' table_name ' BASE=&trglib..' table_name '(&DB_BL_OPTS) FORCE;';
                PUT +6 'RUN;';
                PUT +3 '%end;';
                PUT +3 '%else %do;';
                PUT +6 'PROC SQL NOERRORSTOP;';
                PUT +9 'INSERT INTO &trglib..' "&table_name." ' (';
                PUT +12 @;
            END;
        ...
        /* SELECT part of the INSERT/SELECT */
        DATA _NULL_;
            SET &column_table. END=last;
            WHERE upcase(table_name) = "&table_name.";
            FILE codefile MOD;
            IF _n_ = 1 THEN PUT +6 'SELECT ' @;
            IF mod(_n_, 4) = 0 THEN DO;
                PUT;
                PUT +12 @;
            END;
            ...
            IF last THEN DO;
                PUT;
                PUT +9 'FROM &udmmart..' table_name ';';
                PUT +6 'QUIT;';
                PUT +3 '%end;';
                PUT +3 '%err_check (Failed to insert into ' table_name ', ' table_name ');';
            END;
        RUN;
    %end;
    ...
    %else %do;
        ...
        /* Step 1: Dedup source, apply timezone conversion */
        DATA _NULL_;
            SET &column_table. END=last;
        ...
        /* NOTE: T-SQL requires a semicolon to terminate MERGE.        */
        /* The ';' here ends the T-SQL statement; ')' closes EXECUTE.  */
        DATA _NULL_;
            SET &column_table. END=last;
            LENGTH staging_table $32;
            WHERE upcase(table_name) = "&table_name.";
