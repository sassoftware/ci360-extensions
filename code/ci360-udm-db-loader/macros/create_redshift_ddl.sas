...
/* ****************************************************************************/
%macro create_redshift_ddl;
%let errFlag = 0;

%put ******create_redshift_ddl****** &target_table_name;
%let pk_col=;
...
filename RDSFTDDL DISK "&code_file_path.";
...
data _null_;
    ...
    PUT+1 'CONNECT TO REDSHIFT (&sql_passthru_connection.);';       
	...
	if "&pk_col" ne '' then do;
		if &col_hash_flag. eq 1 then do;
			put +6 'execute (alter table &dbschema..' %trim("&target_table_name") ' add constraint ' %trim("&pk_name") ' primary key (' %trim("&hash_column") ')) BY REDSHIFT;';
		end;
		else do;
			put +6 'execute (alter table &dbschema..' %trim("&target_table_name") ' add constraint ' %trim("&pk_name") ' primary key (' %trim("&pk_col") ')) BY REDSHIFT;';
		end;
	end;
/*	PUT+6 'EXECUTE ( ALTER TABLE '"&target_table_name"' ADD CONSTRAINT  '"&pk_name"'  PRIMARY KEY ('%trim("&pk_col")')) BY REDSHIFT;';*/

	   
    PUT+1 'DISCONNECT FROM REDSHIFT;';
    PUT+1 'QUIT;';
...
filename RDSFTDDL;
filename _all_ list;
%ErrCheck (Unable to generate ddl code for REDSHIFT,create_redshift_ddl);

%mend create_redshift_ddl;
/*%create_mssql_ddl;*/
