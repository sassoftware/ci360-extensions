...
/* ****************************************************************************/
%macro create_oracle_ddl;
%let errFlag = 0;

%put ******create_oracle_ddl****** &target_table_name;
%let pk_col=;
...
filename ORCLDDL DISK "&code_file_path.";
...
data _null_;
    ...
    PUT+1 'connect to ORACLE (&sql_passthru_connection.);';       
	...
	if "&pk_col" ne '' then do;
		if &col_hash_flag. eq 1 then do;
			put +6 'execute (alter table &dbschema..' %trim("&target_table_name") ' add constraint ' %trim("&pk_name") ' primary key (' %trim("&hash_column") ')) by ORACLE;';
		end;
		else do;
			put +6 'execute (alter table &dbschema..' %trim("&target_table_name") ' add constraint ' %trim("&pk_name") ' primary key (' %trim("&pk_col") ')) by ORACLE;';
		end;
	end;
/*	PUT+6 'EXECUTE ( ALTER TABLE '"&target_table_name"' ADD CONSTRAINT  '"&pk_name"'  PRIMARY KEY ('%trim("&pk_col")')) BY ORACLE;';*/
...
filename ORCLDDL;
filename _all_ list;
%ErrCheck (Unable to generate ddl code for oracle,create_oracle_ddl);

%mend create_oracle_ddl;
/*%create_oracle_ddl;*/
