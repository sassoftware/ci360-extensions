...
%if %sysfunc(exist(&udmmart..ABT_ATTRIBUTION)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         ...
         EXECUTE (MERGE INTO &dbschema..ABT_ATTRIBUTION USING &tmpdbschema..ABT_ATTRIBUTION_tmp AS d ON (
            ABT_ATTRIBUTION.interaction_dttm = d.interaction_dttm AND 
            ABT_ATTRIBUTION.interaction_id = d.interaction_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..AB_TEST_PATH_ASSIGNMENT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..AB_TEST_PATH_ASSIGNMENT USING &tmpdbschema..AB_TEST_PATH_ASSIGNMENT_tmp AS d ON (
            AB_TEST_PATH_ASSIGNMENT.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..ACTIVITY_CONVERSION)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..ACTIVITY_CONVERSION USING &tmpdbschema..ACTIVITY_CONVERSION_tmp AS d ON (
            ACTIVITY_CONVERSION.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..ACTIVITY_FLOW_IN)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..ACTIVITY_FLOW_IN USING &tmpdbschema..ACTIVITY_FLOW_IN_tmp AS d ON (
            ACTIVITY_FLOW_IN.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..ACTIVITY_START)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..ACTIVITY_START USING &tmpdbschema..ACTIVITY_START_tmp AS d ON (
            ACTIVITY_START.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..ADVERTISING_CONTACT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..ADVERTISING_CONTACT USING &tmpdbschema..ADVERTISING_CONTACT_tmp AS d ON (
            ADVERTISING_CONTACT.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            load_dttm = d.load_dttm, 
...
%if %sysfunc(exist(&udmmart..AUDIENCE_MEMBERSHIP_CHANGE)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..AUDIENCE_MEMBERSHIP_CHANGE USING &tmpdbschema..AUDIENCE_MEMBERSHIP_CHANGE_tmp AS d ON (
            AUDIENCE_MEMBERSHIP_CHANGE.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..BUSINESS_PROCESS_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..BUSINESS_PROCESS_DETAILS USING &tmpdbschema..BUSINESS_PROCESS_DETAILS_tmp AS d ON (
            BUSINESS_PROCESS_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..CART_ACTIVITY_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..CART_ACTIVITY_DETAILS USING &tmpdbschema..CART_ACTIVITY_DETAILS_tmp AS d ON (
            CART_ACTIVITY_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..CDM_CONTACT_HISTORY)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..CDM_CONTACT_HISTORY USING &tmpdbschema..CDM_CONTACT_HISTORY_tmp AS d ON (
            CDM_CONTACT_HISTORY.contact_id = d.contact_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..CDM_RESPONSE_EXTENDED_ATTR)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..CDM_RESPONSE_EXTENDED_ATTR USING &tmpdbschema..CDM_RESPONSE_EXTENDED_ATTR_tmp AS d ON (
            CDM_RESPONSE_EXTENDED_ATTR.response_id = d.response_id AND 
            CDM_RESPONSE_EXTENDED_ATTR.response_attribute_type_cd = d.response_attribute_type_cd AND CDM_RESPONSE_EXTENDED_ATTR.attribute_nm = d.attribute_nm )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..CDM_RESPONSE_HISTORY)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..CDM_RESPONSE_HISTORY USING &tmpdbschema..CDM_RESPONSE_HISTORY_tmp AS d ON (
            CDM_RESPONSE_HISTORY.response_id = d.response_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..CONTACT_HISTORY)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..CONTACT_HISTORY USING &tmpdbschema..CONTACT_HISTORY_tmp AS d ON (
            CONTACT_HISTORY.contact_id = d.contact_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..CONVERSION_MILESTONE)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..CONVERSION_MILESTONE USING &tmpdbschema..CONVERSION_MILESTONE_tmp AS d ON (
            CONVERSION_MILESTONE.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            test_flg = d.test_flg, 
...
%if %sysfunc(exist(&udmmart..CUSTOM_EVENTS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..CUSTOM_EVENTS USING &tmpdbschema..CUSTOM_EVENTS_tmp AS d ON (
            CUSTOM_EVENTS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..CUSTOM_EVENTS_EXT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..CUSTOM_EVENTS_EXT USING &tmpdbschema..CUSTOM_EVENTS_EXT_tmp AS d ON (
            CUSTOM_EVENTS_EXT.custom_events_sk = d.custom_events_sk )
         WHEN MATCHED THEN
         UPDATE SET
            custom_revenue_amt = d.custom_revenue_amt, 
...
%if %sysfunc(exist(&udmmart..DAILY_USAGE)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DAILY_USAGE USING &tmpdbschema..DAILY_USAGE_tmp AS d ON (
            DAILY_USAGE.event_day = d.event_day )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DATA_VIEW_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DATA_VIEW_DETAILS USING &tmpdbschema..DATA_VIEW_DETAILS_tmp AS d ON (
            DATA_VIEW_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DBT_ADV_CAMPAIGN_VISITORS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DBT_ADV_CAMPAIGN_VISITORS USING &tmpdbschema..DBT_ADV_CAMPAIGN_VISITORS_tmp AS d ON (
            DBT_ADV_CAMPAIGN_VISITORS.visit_id = d.visit_id AND 
            DBT_ADV_CAMPAIGN_VISITORS.session_id = d.session_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DBT_BUSINESS_PROCESS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DBT_BUSINESS_PROCESS USING &tmpdbschema..DBT_BUSINESS_PROCESS_tmp AS d ON (
            DBT_BUSINESS_PROCESS.bus_process_started_dttm = d.bus_process_started_dttm AND 
            DBT_BUSINESS_PROCESS.session_id = d.session_id AND DBT_BUSINESS_PROCESS.business_process_step_name = d.business_process_step_name AND 
            DBT_BUSINESS_PROCESS.business_process_name = d.business_process_name )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DBT_CONTENT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DBT_CONTENT USING &tmpdbschema..DBT_CONTENT_tmp AS d ON (
            DBT_CONTENT.detail_id = d.detail_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DBT_DOCUMENTS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DBT_DOCUMENTS USING &tmpdbschema..DBT_DOCUMENTS_tmp AS d ON (
            DBT_DOCUMENTS.detail_id = d.detail_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DBT_ECOMMERCE)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DBT_ECOMMERCE USING &tmpdbschema..DBT_ECOMMERCE_tmp AS d ON (
            DBT_ECOMMERCE.product_activity_dttm = d.product_activity_dttm AND 
            DBT_ECOMMERCE.visit_id = d.visit_id AND DBT_ECOMMERCE.product_sku = d.product_sku AND 
            DBT_ECOMMERCE.product_name = d.product_name AND DBT_ECOMMERCE.product_id = d.product_id AND 
            DBT_ECOMMERCE.basket_id = d.basket_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DBT_FORMS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DBT_FORMS USING &tmpdbschema..DBT_FORMS_tmp AS d ON (
            DBT_FORMS.detail_id = d.detail_id )
         WHEN MATCHED THEN
         UPDATE SET
            attempts = d.attempts, 
...
%if %sysfunc(exist(&udmmart..DBT_GOALS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DBT_GOALS USING &tmpdbschema..DBT_GOALS_tmp AS d ON (
            DBT_GOALS.goal_name = d.goal_name AND 
            DBT_GOALS.goal_group_name = d.goal_group_name AND DBT_GOALS.detail_id = d.detail_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DBT_MEDIA_CONSUMPTION)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DBT_MEDIA_CONSUMPTION USING &tmpdbschema..DBT_MEDIA_CONSUMPTION_tmp AS d ON (
            DBT_MEDIA_CONSUMPTION.maximum_progress = d.maximum_progress AND 
            DBT_MEDIA_CONSUMPTION.interactions_count = d.interactions_count AND DBT_MEDIA_CONSUMPTION.visit_id = d.visit_id AND 
            DBT_MEDIA_CONSUMPTION.media_section = d.media_section AND DBT_MEDIA_CONSUMPTION.media_completion_rate = d.media_completion_rate AND 
            DBT_MEDIA_CONSUMPTION.detail_id = d.detail_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DBT_PROMOTIONS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DBT_PROMOTIONS USING &tmpdbschema..DBT_PROMOTIONS_tmp AS d ON (
            DBT_PROMOTIONS.detail_id = d.detail_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DBT_SEARCH)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DBT_SEARCH USING &tmpdbschema..DBT_SEARCH_tmp AS d ON (
            DBT_SEARCH.detail_id = d.detail_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DIRECT_CONTACT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DIRECT_CONTACT USING &tmpdbschema..DIRECT_CONTACT_tmp AS d ON (
            DIRECT_CONTACT.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DOCUMENT_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DOCUMENT_DETAILS USING &tmpdbschema..DOCUMENT_DETAILS_tmp AS d ON (
            DOCUMENT_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            load_dttm = d.load_dttm, 
...
%if %sysfunc(exist(&udmmart..EMAIL_BOUNCE)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..EMAIL_BOUNCE USING &tmpdbschema..EMAIL_BOUNCE_tmp AS d ON (
            EMAIL_BOUNCE.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            test_flg = d.test_flg, 
            properties_map_doc = d.properties_map_doc, load_dttm = d.load_dttm, 
...
%if %sysfunc(exist(&udmmart..EMAIL_CLICK)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..EMAIL_CLICK USING &tmpdbschema..EMAIL_CLICK_tmp AS d ON (
            EMAIL_CLICK.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            test_flg = d.test_flg, 
...
%if %sysfunc(exist(&udmmart..EMAIL_COMPLAINT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..EMAIL_COMPLAINT USING &tmpdbschema..EMAIL_COMPLAINT_tmp AS d ON (
            EMAIL_COMPLAINT.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            test_flg = d.test_flg, 
            properties_map_doc = d.properties_map_doc, load_dttm = d.load_dttm, 
...
%if %sysfunc(exist(&udmmart..EMAIL_OPEN)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..EMAIL_OPEN USING &tmpdbschema..EMAIL_OPEN_tmp AS d ON (
            EMAIL_OPEN.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..EMAIL_OPTOUT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..EMAIL_OPTOUT USING &tmpdbschema..EMAIL_OPTOUT_tmp AS d ON (
            EMAIL_OPTOUT.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            test_flg = d.test_flg, 
...
%if %sysfunc(exist(&udmmart..EMAIL_OPTOUT_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..EMAIL_OPTOUT_DETAILS USING &tmpdbschema..EMAIL_OPTOUT_DETAILS_tmp AS d ON (
            EMAIL_OPTOUT_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            test_flg = d.test_flg, 
...
%if %sysfunc(exist(&udmmart..EMAIL_REPLY)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..EMAIL_REPLY USING &tmpdbschema..EMAIL_REPLY_tmp AS d ON (
            EMAIL_REPLY.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            test_flg = d.test_flg, 
...
%if %sysfunc(exist(&udmmart..EMAIL_SEND)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..EMAIL_SEND USING &tmpdbschema..EMAIL_SEND_tmp AS d ON (
            EMAIL_SEND.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            test_flg = d.test_flg, 
            properties_map_doc = d.properties_map_doc, load_dttm = d.load_dttm, 
...
%if %sysfunc(exist(&udmmart..EMAIL_VIEW)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..EMAIL_VIEW USING &tmpdbschema..EMAIL_VIEW_tmp AS d ON (
            EMAIL_VIEW.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            test_flg = d.test_flg, 
            properties_map_doc = d.properties_map_doc, load_dttm = d.load_dttm, 
...
%if %sysfunc(exist(&udmmart..EXTERNAL_EVENT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..EXTERNAL_EVENT USING &tmpdbschema..EXTERNAL_EVENT_tmp AS d ON (
            EXTERNAL_EVENT.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..FORM_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..FORM_DETAILS USING &tmpdbschema..FORM_DETAILS_tmp AS d ON (
            FORM_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..IDENTITY_ATTRIBUTES)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..IDENTITY_ATTRIBUTES USING &tmpdbschema..IDENTITY_ATTRIBUTES_tmp AS d ON (
            IDENTITY_ATTRIBUTES.entrytime = d.entrytime AND 
            IDENTITY_ATTRIBUTES.user_identifier_val = d.user_identifier_val AND IDENTITY_ATTRIBUTES.identifier_type_id = d.identifier_type_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..IDENTITY_MAP)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..IDENTITY_MAP USING &tmpdbschema..IDENTITY_MAP_tmp AS d ON (
            IDENTITY_MAP.source_identity_id = d.source_identity_id )
         WHEN MATCHED THEN
         UPDATE SET
            processed_dttm = d.processed_dttm, 
...
%if %sysfunc(exist(&udmmart..IMPRESSION_DELIVERED)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..IMPRESSION_DELIVERED USING &tmpdbschema..IMPRESSION_DELIVERED_tmp AS d ON (
            IMPRESSION_DELIVERED.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            control_group_flg = d.control_group_flg, 
...
%if %sysfunc(exist(&udmmart..IMPRESSION_SPOT_VIEWABLE)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..IMPRESSION_SPOT_VIEWABLE USING &tmpdbschema..IMPRESSION_SPOT_VIEWABLE_tmp AS d ON (
            IMPRESSION_SPOT_VIEWABLE.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            control_group_flg = d.control_group_flg, 
            product_qty_no = d.product_qty_no, properties_map_doc = d.properties_map_doc, 
...
%if %sysfunc(exist(&udmmart..IN_APP_FAILED)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..IN_APP_FAILED USING &tmpdbschema..IN_APP_FAILED_tmp AS d ON (
            IN_APP_FAILED.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            properties_map_doc = d.properties_map_doc, 
...
%if %sysfunc(exist(&udmmart..IN_APP_MESSAGE)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..IN_APP_MESSAGE USING &tmpdbschema..IN_APP_MESSAGE_tmp AS d ON (
            IN_APP_MESSAGE.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            properties_map_doc = d.properties_map_doc, 
...
%if %sysfunc(exist(&udmmart..IN_APP_SEND)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..IN_APP_SEND USING &tmpdbschema..IN_APP_SEND_tmp AS d ON (
            IN_APP_SEND.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            properties_map_doc = d.properties_map_doc, 
...
%if %sysfunc(exist(&udmmart..IN_APP_TARGETING_REQUEST)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..IN_APP_TARGETING_REQUEST USING &tmpdbschema..IN_APP_TARGETING_REQUEST_tmp AS d ON (
            IN_APP_TARGETING_REQUEST.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..JOURNEY_ENTRY)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..JOURNEY_ENTRY USING &tmpdbschema..JOURNEY_ENTRY_tmp AS d ON (
            JOURNEY_ENTRY.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..JOURNEY_EXIT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..JOURNEY_EXIT USING &tmpdbschema..JOURNEY_EXIT_tmp AS d ON (
            JOURNEY_EXIT.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..JOURNEY_HOLDOUT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..JOURNEY_HOLDOUT USING &tmpdbschema..JOURNEY_HOLDOUT_tmp AS d ON (
            JOURNEY_HOLDOUT.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..JOURNEY_NODE_ENTRY)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..JOURNEY_NODE_ENTRY USING &tmpdbschema..JOURNEY_NODE_ENTRY_tmp AS d ON (
            JOURNEY_NODE_ENTRY.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..JOURNEY_SUCCESS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..JOURNEY_SUCCESS USING &tmpdbschema..JOURNEY_SUCCESS_tmp AS d ON (
            JOURNEY_SUCCESS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            unit_qty = d.unit_qty, 
...
%if %sysfunc(exist(&udmmart..JOURNEY_SUPPRESSION)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..JOURNEY_SUPPRESSION USING &tmpdbschema..JOURNEY_SUPPRESSION_tmp AS d ON (
            JOURNEY_SUPPRESSION.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            load_dttm = d.load_dttm, 
...
%if %sysfunc(exist(&udmmart..JOURNEY_TEST_SUCCESS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..JOURNEY_TEST_SUCCESS USING &tmpdbschema..JOURNEY_TEST_SUCCESS_tmp AS d ON (
            JOURNEY_TEST_SUCCESS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..MEDIA_ACTIVITY_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..MEDIA_ACTIVITY_DETAILS USING &tmpdbschema..MEDIA_ACTIVITY_DETAILS_tmp AS d ON (
            MEDIA_ACTIVITY_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..MEDIA_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..MEDIA_DETAILS USING &tmpdbschema..MEDIA_DETAILS_tmp AS d ON (
            MEDIA_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..MEDIA_DETAILS_EXT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..MEDIA_DETAILS_EXT USING &tmpdbschema..MEDIA_DETAILS_EXT_tmp AS d ON (
            MEDIA_DETAILS_EXT.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..MOBILE_FOCUS_DEFOCUS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..MOBILE_FOCUS_DEFOCUS USING &tmpdbschema..MOBILE_FOCUS_DEFOCUS_tmp AS d ON (
            MOBILE_FOCUS_DEFOCUS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..MOBILE_SPOTS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..MOBILE_SPOTS USING &tmpdbschema..MOBILE_SPOTS_tmp AS d ON (
            MOBILE_SPOTS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            action_dttm_tz = d.action_dttm_tz, 
            action_dttm = d.action_dttm, load_dttm = d.load_dttm, 
...
%if %sysfunc(exist(&udmmart..MONTHLY_USAGE)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..MONTHLY_USAGE USING &tmpdbschema..MONTHLY_USAGE_tmp AS d ON (
            MONTHLY_USAGE.event_month = d.event_month )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..NOTIFICATION_FAILED)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..NOTIFICATION_FAILED USING &tmpdbschema..NOTIFICATION_FAILED_tmp AS d ON (
            NOTIFICATION_FAILED.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            properties_map_doc = d.properties_map_doc, 
...
%if %sysfunc(exist(&udmmart..NOTIFICATION_OPENED)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..NOTIFICATION_OPENED USING &tmpdbschema..NOTIFICATION_OPENED_tmp AS d ON (
            NOTIFICATION_OPENED.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            properties_map_doc = d.properties_map_doc, 
...
%if %sysfunc(exist(&udmmart..NOTIFICATION_SEND)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..NOTIFICATION_SEND USING &tmpdbschema..NOTIFICATION_SEND_tmp AS d ON (
            NOTIFICATION_SEND.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            properties_map_doc = d.properties_map_doc, 
...
%if %sysfunc(exist(&udmmart..NOTIFICATION_TARGETING_REQUEST)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..NOTIFICATION_TARGETING_REQUEST USING &tmpdbschema..NOTIFICATION_TARGETING_REQUE_tmp AS d ON (
            NOTIFICATION_TARGETING_REQUEST.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            eligibility_flg = d.eligibility_flg, 
...
%if %sysfunc(exist(&udmmart..ORDER_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..ORDER_DETAILS USING &tmpdbschema..ORDER_DETAILS_tmp AS d ON (
            ORDER_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            unit_price_amt = d.unit_price_amt, 
...
%if %sysfunc(exist(&udmmart..ORDER_SUMMARY)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..ORDER_SUMMARY USING &tmpdbschema..ORDER_SUMMARY_tmp AS d ON (
            ORDER_SUMMARY.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..OUTBOUND_SYSTEM)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..OUTBOUND_SYSTEM USING &tmpdbschema..OUTBOUND_SYSTEM_tmp AS d ON (
            OUTBOUND_SYSTEM.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            properties_map_doc = d.properties_map_doc, 
...
%if %sysfunc(exist(&udmmart..PAGE_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..PAGE_DETAILS USING &tmpdbschema..PAGE_DETAILS_tmp AS d ON (
            PAGE_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..PAGE_DETAILS_EXT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..PAGE_DETAILS_EXT USING &tmpdbschema..PAGE_DETAILS_EXT_tmp AS d ON (
            PAGE_DETAILS_EXT.load_dttm = d.load_dttm AND 
            PAGE_DETAILS_EXT.session_id = d.session_id AND PAGE_DETAILS_EXT.detail_id = d.detail_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..PAGE_ERRORS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..PAGE_ERRORS USING &tmpdbschema..PAGE_ERRORS_tmp AS d ON (
            PAGE_ERRORS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..PRODUCT_VIEWS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..PRODUCT_VIEWS USING &tmpdbschema..PRODUCT_VIEWS_tmp AS d ON (
            PRODUCT_VIEWS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..PROMOTION_DISPLAYED)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..PROMOTION_DISPLAYED USING &tmpdbschema..PROMOTION_DISPLAYED_tmp AS d ON (
            PROMOTION_DISPLAYED.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..PROMOTION_USED)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..PROMOTION_USED USING &tmpdbschema..PROMOTION_USED_tmp AS d ON (
            PROMOTION_USED.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..RESPONSE_HISTORY)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..RESPONSE_HISTORY USING &tmpdbschema..RESPONSE_HISTORY_tmp AS d ON (
            RESPONSE_HISTORY.response_id = d.response_id )
         WHEN MATCHED THEN
         UPDATE SET
            properties_map_doc = d.properties_map_doc, 
...
%if %sysfunc(exist(&udmmart..SEARCH_RESULTS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SEARCH_RESULTS USING &tmpdbschema..SEARCH_RESULTS_tmp AS d ON (
            SEARCH_RESULTS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..SEARCH_RESULTS_EXT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SEARCH_RESULTS_EXT USING &tmpdbschema..SEARCH_RESULTS_EXT_tmp AS d ON (
            SEARCH_RESULTS_EXT.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..SESSION_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SESSION_DETAILS USING &tmpdbschema..SESSION_DETAILS_tmp AS d ON (
            SESSION_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..SESSION_DETAILS_EXT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SESSION_DETAILS_EXT USING &tmpdbschema..SESSION_DETAILS_EXT_tmp AS d ON (
            SESSION_DETAILS_EXT.last_session_activity_dttm = d.last_session_activity_dttm AND 
            SESSION_DETAILS_EXT.session_id = d.session_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..SMS_MESSAGE_CLICKED)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SMS_MESSAGE_CLICKED USING &tmpdbschema..SMS_MESSAGE_CLICKED_tmp AS d ON (
            SMS_MESSAGE_CLICKED.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..SMS_MESSAGE_DELIVERED)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SMS_MESSAGE_DELIVERED USING &tmpdbschema..SMS_MESSAGE_DELIVERED_tmp AS d ON (
            SMS_MESSAGE_DELIVERED.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..SMS_MESSAGE_FAILED)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SMS_MESSAGE_FAILED USING &tmpdbschema..SMS_MESSAGE_FAILED_tmp AS d ON (
            SMS_MESSAGE_FAILED.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..SMS_MESSAGE_REPLY)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SMS_MESSAGE_REPLY USING &tmpdbschema..SMS_MESSAGE_REPLY_tmp AS d ON (
            SMS_MESSAGE_REPLY.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            load_dttm = d.load_dttm, 
...
%if %sysfunc(exist(&udmmart..SMS_MESSAGE_SEND)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SMS_MESSAGE_SEND USING &tmpdbschema..SMS_MESSAGE_SEND_tmp AS d ON (
            SMS_MESSAGE_SEND.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..SMS_OPTOUT)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SMS_OPTOUT USING &tmpdbschema..SMS_OPTOUT_tmp AS d ON (
            SMS_OPTOUT.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            load_dttm = d.load_dttm, 
...
%if %sysfunc(exist(&udmmart..SMS_OPTOUT_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SMS_OPTOUT_DETAILS USING &tmpdbschema..SMS_OPTOUT_DETAILS_tmp AS d ON (
            SMS_OPTOUT_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            load_dttm = d.load_dttm, 
            sms_optout_dttm = d.sms_optout_dttm, sms_optout_dttm_tz = d.sms_optout_dttm_tz, 
...
%if %sysfunc(exist(&udmmart..SPOT_CLICKED)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SPOT_CLICKED USING &tmpdbschema..SPOT_CLICKED_tmp AS d ON (
            SPOT_CLICKED.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            control_group_flg = d.control_group_flg, 
            product_qty_no = d.product_qty_no, properties_map_doc = d.properties_map_doc, 
...
%if %sysfunc(exist(&udmmart..SPOT_REQUESTED)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SPOT_REQUESTED USING &tmpdbschema..SPOT_REQUESTED_tmp AS d ON (
            SPOT_REQUESTED.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
            properties_map_doc = d.properties_map_doc, 
...
%if %sysfunc(exist(&udmmart..VISIT_DETAILS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..VISIT_DETAILS USING &tmpdbschema..VISIT_DETAILS_tmp AS d ON (
            VISIT_DETAILS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
