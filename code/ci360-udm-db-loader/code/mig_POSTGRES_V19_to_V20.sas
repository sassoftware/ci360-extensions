...
PROC SQL;
CONNECT TO &database. (&sql_passthru_connection);
   EXECUTE (ALTER TABLE &dbschema..DAILY_USAGE  ADD dm_destinations_total_id_cnt  numeric  NULL) BY &database.;
   EXECUTE (ALTER TABLE &dbschema..DAILY_USAGE  ADD dm_destinations_total_row_cnt  numeric  NULL) BY &database.;
DISCONNECT FROM &database.;
QUIT;
...
PROC SQL;
CONNECT TO &database. (&sql_passthru_connection);
   EXECUTE (ALTER TABLE &dbschema..MONTHLY_USAGE  ADD dm_destinations_total_id_cnt  numeric  NULL) BY &database.;
   EXECUTE (ALTER TABLE &dbschema..MONTHLY_USAGE  ADD dm_destinations_total_row_cnt  numeric  NULL) BY &database.;
DISCONNECT FROM &database.;
QUIT;
%err_check (Failed to alter Table: &table_name., &table_name.);
