   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         ...
         EXECUTE (MERGE INTO &dbschema..DBT_BUSINESS_PROCESS USING &tmpdbschema..DBT_BUSINESS_PROCESS_tmp AS d ON (
            ...
            DBT_BUSINESS_PROCESS.session_id = d.session_id AND DBT_BUSINESS_PROCESS.business_process_name = d.business_process_name AND 
            DBT_BUSINESS_PROCESS.business_process_step_name = d.business_process_step_name )
         WHEN MATCHED THEN
         UPDATE SET
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         ...
         EXECUTE (MERGE INTO &dbschema..DBT_ECOMMERCE USING &tmpdbschema..DBT_ECOMMERCE_tmp AS d ON (
            ...
            DBT_ECOMMERCE.product_name = d.product_name AND DBT_ECOMMERCE.basket_id = d.basket_id AND 
            DBT_ECOMMERCE.product_id = d.product_id )
         WHEN MATCHED THEN
         UPDATE SET
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         ...
         EXECUTE (MERGE INTO &dbschema..DBT_MEDIA_CONSUMPTION USING &tmpdbschema..DBT_MEDIA_CONSUMPTION_tmp AS d ON (
            ...
            DBT_MEDIA_CONSUMPTION.detail_id = d.detail_id AND DBT_MEDIA_CONSUMPTION.media_completion_rate = d.media_completion_rate AND 
            DBT_MEDIA_CONSUMPTION.media_section = d.media_section )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..DECISION_EXECUTION)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..DECISION_EXECUTION USING &tmpdbschema..DECISION_EXECUTION_tmp AS d ON (
            DECISION_EXECUTION.execution_id = d.execution_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..EVENT_ERRORS)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..EVENT_ERRORS USING &tmpdbschema..EVENT_ERRORS_tmp AS d ON (
            EVENT_ERRORS.event_id = d.event_id )
         WHEN MATCHED THEN
         UPDATE SET
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         ...
         EXECUTE (MERGE INTO &dbschema..PAGE_DETAILS_EXT USING &tmpdbschema..PAGE_DETAILS_EXT_tmp AS d ON (
            ...
            PAGE_DETAILS_EXT.detail_id = d.detail_id AND PAGE_DETAILS_EXT.session_id = d.session_id )
         WHEN MATCHED THEN
         UPDATE SET
...
%if %sysfunc(exist(&udmmart..SEGMENT_MEMBERSHIP)) %then %do;
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         CONNECT TO &database. (&sql_passthru_connection.);
         EXECUTE (MERGE INTO &dbschema..SEGMENT_MEMBERSHIP USING &tmpdbschema..SEGMENT_MEMBERSHIP_tmp AS d ON (
            SEGMENT_MEMBERSHIP.occurrence_id = d.occurrence_id AND 
            SEGMENT_MEMBERSHIP.segment_version_id = d.segment_version_id AND SEGMENT_MEMBERSHIP.user_identifier_val = d.user_identifier_val )
         WHEN MATCHED THEN
         UPDATE SET
            processed_dttm = d.processed_dttm, 
   ...
   %if &errFlag = 0 %then %do;
      PROC SQL NOERRORSTOP;
         ...
         EXECUTE (MERGE INTO &dbschema..SESSION_DETAILS_EXT USING &tmpdbschema..SESSION_DETAILS_EXT_tmp AS d ON (
            SESSION_DETAILS_EXT.load_dttm = d.load_dttm AND 
